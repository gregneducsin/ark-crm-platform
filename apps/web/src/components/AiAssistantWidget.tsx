  ...
  return (
    <>
      {open && (
        <div className="fixed bottom-20 right-6 z-50 flex h-[520px] w-96 flex-col rounded-lg border border-gray-200 bg-white shadow-xl">
          <div className="flex items-center justify-between border-b border-gray-200 px-4 py-3">
            <p className="text-sm font-semibold text-ark-ink">Ask about your data</p>
            ...
            {messages.map((m, i) => (
              <div key={i} className={m.role === "user" ? "text-right" : "text-left"}>
                <span
                  className={
                    m.role === "user"
                      ? "inline-block whitespace-pre-wrap rounded-lg bg-ark-blue-600 px-3 py-2 text-left text-sm text-white"
          ...
          <form onSubmit={handleSubmit} className="flex items-center gap-2 border-t border-gray-200 p-3">
            <input
              className="flex-1 rounded-md border border-gray-300 px-3 py-1.5 text-sm focus:border-ark-blue-500 focus:outline-none focus:ring-1 focus:ring-ark-blue-500"
      ...
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-ark-blue-600 text-2xl text-white shadow-lg hover:bg-ark-blue-700"
